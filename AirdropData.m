classdef (Abstract) AirdropData < handle & matlab.mixin.Copyable
    ...
    methods (Static)
        function [date] = getdate()
            ...
            timenow = datetime('now', 'TimeZone', 'UTC', 'Format', 'yyyy-MM-dd''T''HH:mm:ssZ');
            date = string(timenow);
        end
        ...
        function [nlines] = countlines(filepath)
            ...
            while ~feof(fID)
                ...
                % characters (ASCII 10)
                nlines = nlines + sum(fread(fID, blocksize, 'char') == newline);
            end
        ...
        function [dataidx] = windowdata(ls, waitboxBool)
            ...
            currylim = ylim(h.ax);
            vertices = [
                leftx,  currylim(1); ...  % Bottom left corner
        ...
        function [dataidx] = fixedwindowdata(ls, windowlength, waitboxBool)
            ...
            widthlistener = addlistener( ...
                dragpatch, ...
                'XData', 'PostSet', ...
                @(s,e)AirdropData.checkdragwindowwidth(e, windowlength) ...
            );
        ...
        function [userchoice] = nbuttondlg(question, buttonlabels, varargin)
            % This is now vendored into this project as a separate *.m file
            % Use https://github.com/StackOverflowMATLABchat/nbuttondlg for issues
            userchoice = nbuttondlg(question, buttonlabels, varargin{:});
        end
